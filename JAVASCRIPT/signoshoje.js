...
const retornaSigno = (colecao_signos) => {
  let data = new Date("2020-02-07 00:00:00");
  let ano = data.getFullYear();
  let dataInicioSigno = new Date(
    ano + "-" + colecao_signos[0]["DataInicio"] + " 00:00:00"
  );
  let dataFimSigno = new Date(
    ano + "-" + colecao_signos[0]["DataFim"] + " 23:59:59"
  );

  if (data >= dataInicioSigno && data <= dataFimSigno) {
    return colecao_signos[0]["Nome"];
  }
};

const nomeSigno = retornaSigno(colecao_signos);
